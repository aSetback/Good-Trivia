...
SlashCmdList["TRIVIA"] = function(message, editbox) 
...
local Questions = {
	...
	{"TV/ Movies: Born Aug 29, 1915, She starred in this movie: Höstsonaten - 1978", "ingrid bergman"},
...
function StartTrivia()
	...
	f:SetScript("OnEvent", ParseChat)
	f:SetScript("OnUpdate", IncrementTimer)
	AskQuestion()
end

-- Stop Trivia
function StopTrivia()
	print("Trivia Stopped.")
	DisableTimer()
	Answer = nil
end
...
function IncrementTimer()
	if (TimerStatus == 1) then
		...
		-- Only check timer if it's running
...
function TimerLimit()
	if TimerAction == "QuestionTime" then
		GuildChat("[Trivia]:  Time is up!  The answer was: " .. Answer)
		Answer = nil
		EnableTimer('AskQuestion', 20)
	end
	
	if TimerAction == "AskQuestion" then
		AskQuestion()
	end
end
...
function DisableTimer()
	...
	TimeLimit = nil
	TimerAction = nil
	TimerStatus = 0 -- Disables incrementing of timer
end

function EnableTimer(Action, Limit)
	Timer = 0 -- Reset timer
	TimeLimit = Limit -- Set time limit for timer
	TimerAction = Action -- Set action for timer when limit is hit
	TimerStatus = 1 -- Starts incrementing of timer
end

-- Ask Trivia Question
function AskQuestion()
	QuestionArray = Questions[ random(1, getn(Questions)) ]
	Question = QuestionArray[1]
	Answer = string.lower(QuestionArray[2])
	GuildChat("[Trivia]: " .. Question .. "?")
	EnableTimer('QuestionTime', 40)
end

-- Parse Guild Chat
function ParseChat()
	Player = arg2
	PlayerAnswer = string.lower(arg1)
	if PlayerAnswer == Answer then
		-- Clear out the answer so no other player can answer
		Answer = nil
		if (Scores[Player] == nil) then
			Scores[Player] = 10
		else
			Scores[Player] = Scores[Player] + 10
		end

		GuildChat("[Trivia]: " .. Player .. " got the answer correct! [Score: " .. Scores[Player] .. "]")
		EnableTimer('AskQuestion', 20)
	end
	
	if arg1 == "-trivia score" then
		ShowScore()
	end

	if arg2 == "Setback" then
		if arg1 == "-trivia question" then
			DisableTimer()
			AskQuestion()
		end
	end
...
-- Show Score in Guild Chat
function ShowScore()
	GuildChat('Scores')
	GuildChat('==============')
	for Player, Score in pairs(Scores) do
		GuildChat(Player .. ': ' .. Score)
	end
	GuildChat('==============')
end

-- Base Commands
function GuildChat(msg)
